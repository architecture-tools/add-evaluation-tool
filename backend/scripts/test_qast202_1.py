...
@enduml"""
    ...
    for base_content, base_components in base_files:
        for variant in range(3):
            ...
            test_files.append(
                (variant_content, base_components, len(variant_content.encode()))
            )
...
def get_auth_token(base_url: str) -> str:
    ...
    test_email = "qast202_test@example.com"
    ...
    raise RuntimeError("Failed to authenticate for QAST202-1 test")
    ...
    for idx, (content, expected_components, file_size) in enumerate(test_files, 1):
        try:
            ...
            if upload_response.status_code != 201:
                print(
                    f"  File {idx}: Upload failed with status {upload_response.status_code}"
                )
                continue
            ...
            if parse_response.status_code == 200:
                ...
                print(
                    f"  ✗ File {idx}: Parse failed with status {parse_response.status_code}"
                )
...
def main() -> int:
    ...
    print(
        f"Processing times under 3s: {under_3s}/{len(processing_times)} ({pct_under_3s:.1f}%)"
    )
    ...
    if success:
        ...
        print(
            f"❌ QAST202-1 FAILED: Only {pct_under_3s:.1f}% processed within 3 seconds (required: 95%)"
        )
